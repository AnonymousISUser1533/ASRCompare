...
class IS(pl.LightningModule):
    def __init__(self,
        ...
        self.text_embedding_model=nn.Embedding(len(self.text_tokenizer)+kmeans, 1024)
    ...
    def forward(self,inputs):
        ...
        for i in range(batchsize):
            ...
            # print(audio[i])
            audio_input=audio[i]+len(self.text_tokenizer)
            audio_input=torch.tensor(audio_input).to(self.device)
            # print(audio_input)
            text_input_pre=self.text_embedding_model(text_pre).squeeze(0)
            text_input_post=self.text_embedding_model(text_post).squeeze(0)
            audio_input=self.text_embedding_model(audio_input).squeeze(0)
        ...
        total_loss = F.cross_entropy(logits, targets, reduction="sum")
        total_loss=total_loss/len_y.sum()
    ...
    def inference(self,inputs):
        ...
        for i in range(batchsize):
            ...
            text_post=text_post[:,:-1]
            # print(text_post)
            #audio[i]的每个token都加tokenizer的size
            # print(audio[i])
            audio_input=audio[i]+len(self.text_tokenizer)
            audio_input=torch.tensor(audio_input).to(self.device)
            # print(audio_input)
